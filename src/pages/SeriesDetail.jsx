...
export default function SeriesDetail() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setActiveSeason(0)
    fetch(`/api/series/detail?url=${encodeURIComponent(url)}`)
      .then(r => r.json())
  ...
  return (
    <div className="page">
      <button className="watch-back" onClick={() => navigate('/series')}>
        ← العودة للمسلسلات
      </button>

      {loading && <Loader text="جاري تحميل تفاصيل المسلسل والحلقات..." />}
                ...
                {data.seasons.map((s, i) => (
                  ...
                  >
                    <div className="season-card-img">
                      ...
                      <div className="season-card-placeholder" style={{ display: s.poster ? 'none' : 'flex' }}>📺</div>
