...
export default function Series() {
  ...
  function applyFilter(type, term) {
    setOpenDrop(null)
    setPage(1)
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h1 className="page-title">المسلسلات</h1>
      </div>
      ...
      <div className="filter-bar" ref={dropRef}>
        <div className="filter-group">
          <div className={`filter-dropdown ${openDrop === 'cat' ? 'open' : ''}`}>
            ...
            {openDrop === 'cat' && (
              <ul className="filter-menu">
                <li className={!selCat ? 'active' : ''} onClick={() => applyFilter('cat', '')}>الكل</li>
                {filters.categories.map(c => (
                  <li key={c.term} className={selCat === c.term ? 'active' : ''} onClick={() => applyFilter('cat', c.term)}>{c.name}</li>
                ))}
              </ul>
            )}
          </div>
          ...
          <div className={`filter-dropdown ${openDrop === 'genre' ? 'open' : ''}`}>
            ...
            {openDrop === 'genre' && (
              <ul className="filter-menu">
                <li className={!selGenre ? 'active' : ''} onClick={() => applyFilter('genre', '')}>الكل</li>
                {filters.genres.map(g => (
                  <li key={g.term} className={selGenre === g.term ? 'active' : ''} onClick={() => applyFilter('genre', g.term)}>{g.name}</li>
                ))}
              </ul>
            )}
          </div>
      ...
      {loading && <Loader text="جاري تحميل المسلسلات..." />}
        ...
        <>
          {data.series?.length === 0 ? (
            ...
            <div className="media-grid">
              {(data.series || []).map((s, i) => (
                <MediaCard key={i} item={s} type="series" />
              ))}
            </div>
          )}
